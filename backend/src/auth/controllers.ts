...
import { authLoginMissingFields, authRegisterMissingFields } from "src/utils/missingFields"
import { userValidator } from "src/validators/userValidator"

const CreateUserController = async (req: Request, res: Response) => {
    if (!bodyRequestHandler(req, res)) return;
    if (!authRegisterMissingFields(req.body, res)) return;
    if (!userValidator(req.body, res)) return;
...
const LoginUserController = async (req: Request, res: Response) => {
    if (!bodyRequestHandler(req, res)) return;
    if (!authLoginMissingFields(req.body, res)) return;
